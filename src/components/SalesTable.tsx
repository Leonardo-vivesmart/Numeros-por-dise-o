        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-zinc-50 border-b border-zinc-200">
              <th className="px-6 py-3 text-xs font-semibold text-zinc-500 uppercase tracking-wider w-32">Mes</th>
              {years.map((year) => (
              ...
              <th className="px-6 py-3 text-xs font-semibold text-zinc-500 uppercase tracking-wider min-w-[120px]">
          ...
          <tbody className="divide-y divide-zinc-200">
            {MONTHS.map((month, monthIndex) => (
              <tr key={month} className="hover:bg-zinc-50/50 transition-colors">
                <td className="px-6 py-4 text-sm font-medium text-zinc-900">{month}</td>
                ...
                <td className="px-6 py-4">
                  {(() => {
                    ...
                    if (!currentYear || !prevYear) return <span className="text-zinc-400">-</span>;
                    
                    const currentVal = salesData[currentYear]?.[monthIndex];
                    const prevVal = salesData[prevYear]?.[monthIndex];
                    
                    if (currentVal === null || currentVal === undefined || prevVal === null || prevVal === undefined) return <span className="text-zinc-400">-</span>;
                ...
                <td className="px-6 py-4">
                  {(() => {
                    ...
                    if (growth === null) return <span className="text-zinc-400">-</span>;
          ...
          <tfoot className="bg-zinc-50 border-t-2 border-zinc-200">
            <tr>
              <td className="px-6 py-4 text-sm font-semibold text-zinc-900">Total Anual</td>
              {years.map((year) => (
                <td key={`total-${year}`} className="px-6 py-4 text-sm font-semibold text-zinc-900">
                  {formatCurrency(calculateAnnualTotal(salesData[year]), companyInfo.currency)}
                </td>
              ))}
              <td className="px-6 py-4">
                {(() => {
            ...
            <tr>
              <td className="px-6 py-4 text-sm font-medium text-zinc-500">Promedio Mensual</td>
              {years.map((year) => (
